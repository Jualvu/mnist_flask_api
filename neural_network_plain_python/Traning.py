...
X = mnist_dataset.iloc[50000:60000, 1:]
y = mnist_dataset.iloc[50000:60000, 0:1]
...
#training variables to consider saving weigths and biases
global_accuracy = 0
save_weights = True
...
for i in range(epochs):
    ...
    for x_sample, y_sample in zip(X_normalized, y):
        ...
        #dE_dY same size as output/y_true
        dE_dY = []

        for y_p, y_t in zip(output, y_true):
            dE_dY.append(y_p - y_t)



        for layer in reversed(layers):
            dE_dY = layer.backward(gradients=dE_dY, learning_rate=learning_rate)

    print(f"\nTotal ERROR: {error}")

    average_loss = error / len(X_normalized)
    accuracy = correct_predictions / len(X_normalized)
    print(f"Epoch {i+1}: Avarage Loss = {average_loss:.4f}, Accuracy = {accuracy * 100:.2f}%\n")
    
    # check accuracy not decreasing
    if accuracy > global_accuracy:  global_accuracy = accuracy
    else: 
        print(f"Accuracy decreasing: stopping training.")
        save_weights = False
        break;
